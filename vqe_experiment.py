...
def run_vqe(n_qubits, coeffs, paulis, param_guess, budget, shots, mode, backend, save_dir, loss_file, params_file, mitigator=None,noise_backend=None,**vqe_kwargs):
    """
...
def run_cafqa(n_qubits, coeffs, paulis, param_guess, budget, shots, mode, backend, save_dir, loss_file, params_file, vqe_kwargs):
    ...
    x_cafqa = None
    with open(config["log_file"]) as f:  #read the output of function 'minimize'
        lines = f.readlines()
        counter = 0
        for idx, line in enumerate(lines[::-1]):  #[::-1]： it stands for reverse order
            #enumerate is useful for obtaining an indexed list:    (0, seq[0]), (1, seq[1]), (2, seq[2]), ...
            if line[:16] == "Best point found" or line[:29] == "Minimum of the posterior mean": #line[:16] the first 16 items,ac
                '''格式大致如下:
                ...
                if energy < energy_cafqa:
                    energy_cafqa = energy
                    x_cafqa = [int(y) for y in parts[:-1]] #record the params of x_i
            if counter == 2:
                break
